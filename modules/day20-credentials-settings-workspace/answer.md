...
**任务3：显式恢复成上一层的值该怎么设计**：直接把用户层设成 `undefined` **不能**达到"恢复成上一层默认值"的效果——按 study.md 第2节真值表最后一行，`Object.entries()` 照样会把这个 key 列出来（JS 对象里"属性存在但值是 `undefined`"和"属性完全不存在"是两种不同状态,前者会被 `Object.entries` 枚举出来）,所以 `mergeSettings` 会把 `result[key]` 覆盖成 `{ value: undefined, source: 'user' }`，而不是"跳过这一层、保留上一层的值"——这跟目标行为正好相反。

真正要支持这种语义，需要一个专门的、跟任何合法配置值都不会混淆的哨兵标记（不能用 `null`——`null` 在很多配置场景里本身就是一个合法的业务值，比如"显式关闭某个可选功能"，用 `null` 当哨兵会制造新的"哪个 null 是哪个意思"的歧义）。第一反应容易想到用一个具名的 `Symbol` 当哨兵——`Object.entries()` 确实能正常看到它（值可以是任意类型，`Symbol` 不例外），`mergeSettings` 只需要多一行 `if (value === RESET_TO_PARENT) continue`。但 `Symbol` 有一个实测过的具体问题：`JSON.stringify({ maxTokens: Symbol('x') })` 的结果是 `'{}'`——这个 key 会被静默丢弃,不报错也不留占位。"用户想在自己的配置里显式写一个'恢复默认'的标记"这个场景,几乎必然意味着这个标记迟早要被写进一份持久化的配置文件（JSON,类比 VSCode 的 `settings.json`）——但 JSON 语法里根本没有 `Symbol` 字面量,没法把它写进配置文件；就算在代码里临时构造出这个 `Symbol` 塞进内存对象,一旦这份配置中途经过任何一次 JSON 序列化（存盘、读盘、以后记进 Session 事件），这个哨兵就会在没人注意到的地方消失,产生"读盘前后行为不一致"这种很难查的坑。

更稳妥的选择是一个专门命名、带命名空间前缀的**字符串常量**，比如 `export const RESET_TO_PARENT = '@@mini-harness/reset-to-parent' as const`——它能被原样写进 JSON 配置文件（`"maxTokens": "@@mini-harness/reset-to-parent"`），完整走完"用户手写配置 → 读盘解析 → 传给 `mergeSettings`"这条真实链路而不丢失；加了命名空间前缀之后，跟任何用户真的想设置的合法值（数字、布尔、正常业务字符串）撞车的概率也趋近于零，达到的效果跟 `Symbol` 想要的"绝对不会跟合法值混淆"是一样的，只是换成了一种扛得住序列化的具体实现。`mergeSettings` 遇到这个哨兵时的行为要专门处理：不是"用这一层的值覆盖"，而是"跳过这一层对这个 key 的写入，让更早一层的 `{value, source}` 继续生效"——这跟 Day19 把三种交互类型显式区分成一个封闭联合类型是同一个思路：用一个专门的、能跟普通合法值区分开的标记去表达"这是一种特殊语义"，只是具体选哪种标记时,还要多想一步"这个标记会不会需要扛过一次序列化"。
