...
### 5.0 前置概念：为什么"断言一次输出"这套测试思路，遇到 LLM 就不够用了

写普通代码的单元测试时，一个隐含前提通常成立："同一个输入，代码永远给出同一个输出"——`add(1, 2)` 今天跑是 `3`，明天跑也是 `3`，所以断言"这次结果应该等于 `3`"就足够了。

一旦流程里有一个真实的 LLM 在做决策（哪怕只是"这一步该调用哪个工具、该说哪句话"），这个前提就不成立了——**同一段 prompt，真实 provider 不同次调用完全可能给出不同的回答**，这是模型本身自带的特性（业内通常说"非确定性"/"变动性"），不是代码写错了。"评测"（业内通用叫法是英文 eval，"给一个模型/系统的任务完成情况打分"就是这个词在说的事）要解决的正是这个新问题：**输出允许变动的前提下，怎么系统性地判断"这个系统做得对不对"**，而不是靠盯着某一次输出、凭印象觉得"看起来还行"。

面对这个问题，现实里有两条不同的应对思路，对应今天要讲的两个机制：

1. **先把"会变动的那部分"换成一个确定性的替身，测其余部分**——这正是这门课从 Day7 起一直在用的 `HeuristicInvestigationAdapter`：同一个查询，永远按写死的规则给出同一个回答。用它跑测试，能确定性地验证"给定这组输入，系统里工具调用、审批流程、收尾逻辑这些*不*依赖模型随机性的部分，有没有按预期工作"——这就是下面 §5.1 的"离线场景集"。它验证的不是"模型这次答得好不好"，是"系统其余部分接得对不对"。
2. **真的接了一个会变动结果的 provider 之后，单跑一次已经不能说明问题**——这次跑通了，可能只是运气好，下次同样的输入可能就错了。要知道"这个任务在这个 prompt/模型组合下，能不能稳定做对"，得跑 N 次、看通过率——这就是下面 §5.2 的"模型抽样评测"。

两者验证的对象不同（"系统的确定性部分对不对" vs "模型本身稳不稳定"），所以是"两个不同的问题"，不是同一件事的两种叫法。
