...
**但要注意边界**：`ToolTimeoutError`/`ToolAbortedError` 不属于上面这类"预期内的业务失败"——超时或被取消不是"模型编造了错误参数"这种可以留给模型自己想办法应对的情况，是运行时层面"这次调用没能安全跑完"的信号。`executeToolCall()` 对这两种错误的处理是**原样往上抛**，`runTurn` 接住后让整个 turn 提前以 `cancelled` 收场，不再处理这一步剩下的工具调用，也不会为这次没跑完的调用伪造一条 isError 结果（exercise.md 任务3就是让你亲手实现这条边界）。
