...
**语义查询是什么，跟文本搜索的区别在哪**：语义查询指的是不靠"文字长得像不像"去找东西，而是真的理解代码的语法/作用域/类型结构去回答问题——比如"这个标识符 `Session` 真正声明在哪一行"。文本搜索（Day15 `search_text`）只会暴力匹配"Session"这几个字符出现的所有地方——注释里提到的、字符串字面量里的、另一个文件里同名但不相关的符号、真正的声明本身，全部混在一起，需要调用方自己去猜哪一条才是真的；语义查询靠一个真正理解这门语言的后端（今天是 `tsserver`）按照编译器自己的解析规则给出精确答案，不会被注释/字符串误导。这对一个"只读代码库调查"类型的 Agent 很关键：它经常要回答"这个函数定义在哪""调用链是什么样"，纯文本搜索遇到常见名字会返回一堆噪音，Agent 得自己读一遍、自己判断——既浪费上下文预算，又容易判断错；有了语义查询，直接拿到一个编译器级别可信的答案。

**LSP（Language Server Protocol）是什么**：微软最早为 VSCode 定的一套开放标准，后来被整个编辑器行业采用。在它出现之前，如果想让"编辑器 X"支持"语言 Y"的跳转到定义/自动补全/查类型错误，得有人专门写一段代码让编辑器深度理解这门语言的内部结构——N 个编辑器、M 种语言，理论上要写 N×M 份高度重复的集成代码。LSP 加了一层间接层，把 N×M 拆成 N+M：每种语言只需要实现一次"语言服务器"（一个独立进程，专门回答"这个符号定义在哪""这行有没有类型错误"这类语义查询），每个编辑器只需要实现一次"LSP 客户端"（懂得用同一套标准协议跟任何语言服务器对话），编辑器本身不需要真的"懂"某门具体语言。这跟"一套标准 API 契约,让多个前端和多个后端不用互相为对方定制"是同一个思路。
