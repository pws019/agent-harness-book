...
## 任务 4：一道思考题（不用写代码）——`IdempotencyStore` 该怎么防止无限增长

`study.md` §4 提到一个诚实记录的缺口：`IdempotencyStore` 内部的 `Map` 只会随着 `claim()` 调用增加，从来没有任何清理机制——服务器只要不重启，这个 `Map` 就会跟收到的请求数量成正比、无限增长下去。设计（不用写完整实现，但至少写出类型签名 + 核心判断逻辑）一个带清理机制的版本，至少回答：

1. 你会选"按时间淘汰（TTL）"还是"按数量上限淘汰（类似 LRU）"，还是两个都要？理由是什么（提示：想一想"现实中客户端最多可能隔多久重试一次同一个请求"和"服务器愿意为这件事付出多大内存代价"这两个约束，谁更容易估计）。
2. 清理动作应该长什么形状——是一个独立的 `setInterval()` 定时扫描,还是"每次 `claim()`/`get()` 被调用时顺手检查一下要不要清理"（惰性清理，不占用额外的定时器）？两种各自的权衡是什么（提示：定时器意味着即使完全没有新请求进来,进程也有一个东西在后台持续跑；惰性清理意味着如果长时间没有任何新请求,过期的记录会一直占着内存,直到下一次有请求进来才会被顺便清掉）。
3. 这跟 `createBacklogWriter`（第6节，挡的是"单条连接短时间内写太快"）是不是同一类问题、能不能直接照抄同一套机制？为什么（提示：一个是"单个连接生命周期内的字节计数"，一个是"跨越整个服务器生命周期、由多少个不同 key 决定的记录数量"，两者的时间尺度和触发条件完全不同）。
...
- [ ] 我写完了任务4的设计方向，明确说了 TTL/LRU 怎么选、清理动作放在哪、为什么不能直接照抄 `createBacklogWriter` 的机制。
