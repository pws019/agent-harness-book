...
**代码怎么对照**：`mini-harness/` 是一个 git 仓库（本目录本身就是），每天结束都打了一个 tag（`day01`…`day30`，30 天全部完成）。卡住了可以 `git diff day0X..day0Y` 看某一天具体改了什么，或者 `git log --oneline` 看整体演进脉络。**建议先自己写，卡住 30-40 分钟再对照参考实现**，不要一上来就抄。
...
| 25 | Web、LSP 与代码运行时能力 | [modules/day25-web-lsp-code-runtime/](modules/day25-web-lsp-code-runtime/) |
| 26 | Subagent——隔离、委派与预算 | [modules/day26-subagent-isolation-delegation-budget/](modules/day26-subagent-isolation-delegation-budget/) |
| 27 | 里程碑四：Workflow——可审计编排 | [modules/day27-milestone-workflow-orchestration/](modules/day27-milestone-workflow-orchestration/) |
| 28 | 遥测、审计与评测体系 | [modules/day28-telemetry-audit-eval/](modules/day28-telemetry-audit-eval/) |
| 29 | 压力、混沌、安全与上线门禁 | [modules/day29-load-chaos-security-gate/](modules/day29-load-chaos-security-gate/) |
| 30 | 里程碑五：毕业设计与架构答辩 | [modules/day30-milestone-graduation/](modules/day30-milestone-graduation/) |

跑完 Day1-30（全部30天），`mini-harness/` 会有一个能跑的调查 CLI（`pnpm cli -- --workspace <目录> --query <关键词> --session-file <路径>`，支持崩溃后从同一份文件恢复）、一条端到端的安全写入链路（`pnpm cli -- propose-edit --workspace <目录> --file <路径> --find <文本> --replace <文本>`：读文件 → 生成新内容 → 展示 diff → 人工审批或权限预设 → 批准才写盘 → 可选跑验证命令）、一个能被真实网络访问的 HTTP 服务器（`createHttpServer()`：create/send/cancel/inspect/events stream，换行分隔 JSON 流协议区分 baseline/increment/terminal/transport-error，幂等 key 防重复提交，慢客户端有硬性积压字节上限）、一个断线自动重连、最终收敛到服务端权威状态的终端客户端（`Conversation` 去重/补洞 + `ReconnectingStream` 自动重连），以及独立于 CLI 之外、工具层已经具备的写文件能力（`edit_file`，带 symlink 逃逸防护和乐观并发控制）、子进程执行能力（`run_command`，argv 数组不经过 shell、env 白名单、超时/取消杀整棵进程树）、后台任务能力（`start_job`/`job_status`/`cancel_job`，跨多次调用追踪一个还在跑的子进程，幂等释放）、命令策略（`CommandPolicy`，白名单+fail-closed）、审批机制（`ApprovalStore`，一次性、绑定具体参数，防重放/防换参数/防过期）和凭据/设置能力（`CredentialRef`/`CredentialStore` 只以不透明引用流动、`mergeSettings` 分层合并带来源、`WorkspaceContext` 让 `run_command` 能安全注入凭据而不经过模型），以及一条能编排多个 Agent 的可审计工作流命令（`pnpm cli -- run-workflow --workspace <目录> --script <脚本路径> --query <关键词>`：脚本调用 `agent()`/`parallel()`/`pipeline()`/`phase()` 拼出一棵计划树，经 `node:vm` 沙箱跑出树本身，再交给 `WorkflowRunner` 真正执行，`agent` 叶子复用 Day26 委派机制，取消保证有界时间收尾），以及一份不侵入已有执行路径的遥测/审计/评测体系（`deriveTelemetry()` 从 Day8 事件日志纯投影出延迟/token/成本报告，`AuditingApprovalStore` 包一层 Day19 `ApprovalStore` 发审计记录，`EvalHarness` 用声明式断言取代手写 `expect(...)`），以及一次跨系统的压力/混沌/安全硬化（Day29——30个真实并发 session、200次工具调用的真实长 stream、模型报错/审计 sink 抛错下的收敛验证、SSRF 与 Workflow 权限提升的红队场景，过程中真的找到并修复了一个此前 28 天都没测出来的 `http-server.ts` bug），以及全课程毕业演示（Day30——`graduation-demo.ts` 把 Day1-29 各自独立建好的每一块第一次真的串成大纲原文那条完整叙事：调查 → 压缩 → 提出修改 → 审批 → 沙箱执行 → 后台验证 → 断线重连 → 模拟重启恢复 → 验证目标 → 输出证据/审计/成本，`pnpm cli -- graduation-demo --workspace <目录> --query <关键词>` 可以直接跑；过程中把 Day13 压缩第一次真正接进了 `Agent`——`Agent.compact()`,此前 `planCompaction()`/`applyCompaction()` 写完就是两个从没被调用过的纯函数）。现在共 563 条测试,覆盖流式组装、请求重试、工具执行管线、Agent loop 终态、取消/释放的竞态、多轮会话记忆、事件日志的开闭配对不变式、持久化崩溃恢复、投影/查询/标题派生、系统提示词组装、Token 计量与预算、上下文压缩、文件系统路径逃逸与并发写冲突、子进程故障注入、后台任务生命周期与幂等释放、跨天红队回归、凭据不泄漏、审批+写入+验证端到端链路、HTTP 服务器端到端流程（真实 socket）、客户端断线重连（8 个种子的故障注入）、Goal 状态机、Skill 按需加载、**真实 `tsserver` 协议客户端**（真实 spawn TypeScript 自带的语言服务进程,不是 mock）、语义查询降级到文本搜索、**一次真实复现的 `node:vm` 沙箱逃逸**（配一条"只传数据不传函数就挡得住"的对照测试）、子 Agent 委派（独立 Session、结构化结果跨边界、权限不能升级、预算 fail-closed）、Workflow 编排（四种节点组合出的真实端到端结果、`maxAgents` fail-closed、卡死子任务下的有界取消、终态竞态唯一性）、遥测/审计/评测（真实 `run_command` + `envCredentials` 注入验证脱敏只在遥测报告里生效、原始事件日志本身不受影响；`AuditingApprovalStore` 失败的消费尝试不产生审计记录；种子随机采样评测的结果确定性可复现）。除了 CLI 和服务器,阶段四还建了按需加载技能 `SkillRegistry`（接入 Day11 `buildSystemPrompt`）、人触发命令 `Command`/`CommandRegistry`、`PlanModeController`（复用 Day19 `PermissionPreset`）、目标状态机 `GoalStore`（模型自称完成不能直接关闭目标,必须过外部 `verify()`）、定时提醒 `scheduleReminder`、网页搜索/抓取工具（`provenance:'untrusted-external'` 标记外部内容）、`SubagentManager`/`DelegationBudget`（父 Agent 委派子 Agent，纯进程内、不需要 HTTP 层）。过程中真实踩到并修复了好几个 bug（数组被意外冻结、`AbortSignal` 被深冻结坏掉、取消检查时机错误、Session 收尾事件漏转发进持久化 store、重复压缩产生互相重叠的区间、崩溃截断后续写把日志粘成乱码、重连信号判断不完整、语义查询定位到注释里的名字、`vm` 跨 realm 场景下 `instanceof Error` 失效、委派预算记账时机错误会让并发上限失效、工作流取消如果改成"等子 Agent 真正退出再定终态"会被卡死子任务无限期拖住、遥测只在 usage 为真值时才累计会让"未上报"和"上报了0"变成同一个结果、客户端先等消息彻底发完再开事件流会让服务端永远不发 terminal、`sink?.append(session.append(...))` 这种可选链写法会连同参数表达式一起短路导致压缩完全没发生过、`graduation-demo.ts` 加入项目后 tsserver 的 `references()` 查询大引用量符号会真实卡死（换一个引用数小的符号就正常）……），这些坑本身就是很好的教材，都记录在对应天数的 `study.md` 里，不是我编的案例。`modules/day21-milestone-secure-agent/security-test-index.md` 是一份汇总 Day15-21 安全属性的 25 条测试索引，`incident-drill.md` 是一次假设的安全事件演练；`modules/day27-milestone-workflow-orchestration/phase-iv-test-index.md` 是同样风格的阶段四（Day22-27）30 条测试索引；`modules/day29-load-chaos-security-gate/release-checklist.md` 是根 `README.md` 18条核心不变式逐条对照当前代码状态的上线自查表（11条通过、7条部分通过，都写了具体边界）；`modules/day30-milestone-graduation/` 是全课程的毕业设计包——`architecture.md`（分层视图+类型复合图）、`invariants.md`（18条不变式逐条落地证据）、`threat-model.md`、`failure-model.md`（系统梳理五层故障模式）、`eval-plan.md`、`runbook.md`、`adr/`（6条架构决策记录）。

阶段一~五全部完成——30 天大纲正式收尾。下面这份"阶段 III～V 路线图"章节保留作为原始大纲的历史对照，实际内容已经全部落地在上面表格链接的 `modules/` 目录里，不要以下面的原始大纲为准。
...
以下内容保留自初版大纲——现在整个 30 天已经全部落地，这份原始大纲纯粹是历史对照，读起来会比 `modules/` 里实际写出来的内容吃力，不要以下面这份原始大纲为准，请看每个阶段链接到的 `modules/dayNN-*/` 目录。
...
| IV. 平台化与编排（已完成） | 22～27 | API、Client、skills、goal、subagent、workflow | 可远程使用的 MiniHarness |
...
### 阶段 IV：平台化与编排（Day 22～27）——**已完成，实际实现见 `modules/day22-http-server-and-streaming/`、`modules/day23-client-and-reconnection/`、`modules/day24-skills-commands-plan-goal-reminder/`、`modules/day25-web-lsp-code-runtime/`、`modules/day26-subagent-isolation-delegation-budget/`、`modules/day27-milestone-workflow-orchestration/`**
...
以下是初版大纲的原始描述，保留作参考对照；实际怎么落地的、跟原始大纲哪里不一样（LSP 走了真协议路线——真实 spawn `tsserver`、实测确认它的 Content-Length 帧协议，不是进程内 TS Compiler API 抄近道，这个选择已经跟用户确认过；"Code runtime 用声明式 bindings，不暴露任意宿主对象"这句话背后有一个真实验证过的反例——`node:vm` 即使只传一个绑定函数也能被逃逸，`CodeRuntime` 提供的是最基本的默认限制，不是真安全边界，这条诚实的缺口在原始大纲的验收标准里没有被提及),请看 [modules/day25-web-lsp-code-runtime/study.md](modules/day25-web-lsp-code-runtime/study.md)，不要以下面这份原始大纲为准。
...
以下是初版大纲的原始描述，保留作参考对照；实际怎么落地的、跟原始大纲哪里不一样（不需要 Day22 的 HTTP 层——委派是纯进程内的问题；"持久化 parent/child/delegation depth/预算"没有做成真正落盘持久化,`DelegationBudget`/`SubagentManager` 都是纯内存实现,进程重启后委派树的状态不会保留,这是延续 Day17 `JobRuntime` 同一个"没有真正的任务恢复"诚实缺口；`maxTokens` 是追溯性的,只影响下一个子 Agent 能不能启动,拦不住一个已经在跑的子 Agent 自己超支),请看 [modules/day26-subagent-isolation-delegation-budget/study.md](modules/day26-subagent-isolation-delegation-budget/study.md)，不要以下面这份原始大纲为准。
...
以下是初版大纲的原始描述，保留作参考对照；实际怎么落地的、跟原始大纲哪里不一样（"对比三种编排"里,只真正实现了第一种（确定性 DSL 构建静态树,`WorkflowRunner` 执行），"模型动态选择工具"这门课从 Day5 起 Agent loop 本身一直就是这种形状,没有单独另建一套；"模型生成 workflow 脚本、执行中途分支"完全没有做,是刻意的可审计性权衡,`answer.md` 里给出了理由,不是遗漏；"有限清理时间"没有做成"保证子 Agent 底层真的已经完全停止运行"这么强的承诺——`WorkflowRunner.cancel()` 选择不等 `dispose()` 真正完成,只保证自己的终态记账在有界时间内落定,这条诚实缺口在原始大纲的验收标准里没有被提及),请看 [modules/day27-milestone-workflow-orchestration/README.md](modules/day27-milestone-workflow-orchestration/README.md)，不要以下面这份原始大纲为准。
...
### 阶段 V：生产验证与毕业设计（Day 28～30）——**全部完成，实际实现见 `modules/day28-telemetry-audit-eval/`、`modules/day29-load-chaos-security-gate/`、`modules/day30-milestone-graduation/`**

<details>
<summary>展开 Day28-30 详情</summary>

以下是初版大纲的原始描述，保留作参考对照；实际怎么落地的、跟原始大纲哪里不一样（Telemetry/Audit/Eval 三者都设计成对 Day8 事件日志的纯派生/对 Day19 `ApprovalStore` 的组合包装，不往 `ToolRegistry`/`LlmAdapter`/`ApprovalStore` 内部加任何钩子——这是"设计两次"之后放弃更侵入方案的结果；"模型抽样评测"今天只能靠一个专门造的种子随机合成 adapter 演示聚合机制本身,这门课至今没有真实的非确定性 provider 能演示更有意义的抽样结果),请看 [modules/day28-telemetry-audit-eval/study.md](modules/day28-telemetry-audit-eval/study.md)，不要以下面这份原始大纲为准。
...
以下是初版大纲的原始描述，保留作参考对照；实际怎么落地的、跟原始大纲哪里不一样（不是"预先定义 30 个新场景",是把已有系统往狠里测,顺带修了一个真实 bug；"磁盘满/网络分区"这类需要真实 OS 级故障注入的场景诚实记录为超出范围,延续 Day18 的诚实缺口记录传统；"混沌"部分聚焦在两个可确定性验证的新点——模型报错的收敛性、telemetry sink 抛错不拖垮主任务——而不是泛泛覆盖大纲列的四种；"安全"部分的 SSRF 不是新发现,是延续 Day18 "policy 默认关闭" 立场、今天第一次用真实场景点名),请看 [modules/day29-load-chaos-security-gate/study.md](modules/day29-load-chaos-security-gate/study.md)，不要以下面这份原始大纲为准。

以下是初版大纲的原始描述，保留作参考对照；实际怎么落地的、跟原始大纲哪里不一样（范围明确限定在 mini-harness 收尾——不是用户自己 `docs/` 下的真实毕业项目,那份 `exercise.md` 里引导单独去做；"最终演示"里"调查多个来源"落地成"扫描 workspace 里的多个文件"，不是"多轮独立调查"（`HeuristicInvestigationAdapter` 本身是单查询设计）；"断线重连"是在一条独立起停的 Day22 服务器上单独跑的场景,不是复用同一个正在做调查的 Agent 实例——`SessionRegistry` 只知道怎么造全新的 Agent；"可运维"这条毕业门禁诚实标成部分通过,没有 SLO/容量假设/告警,这门课"零新增运行时依赖"的路线决定了这里没有接任何真实运维基础设施),请看 [modules/day30-milestone-graduation/README.md](modules/day30-milestone-graduation/README.md)，不要以下面这份原始大纲为准。
...
Day1-30（全部30天）的 `modules/` 已经把这套流程内化进了 `study.md`/`exercise.md` 的结构里；如果你要在这个基础上继续探索新的子系统/自己的毕业项目，也建议按这条闭环走：
...
毕业项目前至少落实以下不变式——30 天全部完成后，`mini-harness/` 对这18条的逐条落地状态（11条完全通过、7条有明确记录过边界的部分通过、0条完全没做）见 [modules/day30-milestone-graduation/invariants.md](modules/day30-milestone-graduation/invariants.md)，不要以本节这份原始清单本身的措辞为准：
