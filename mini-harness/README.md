...
每天的学习里程碑都会打一个 git tag（`day01` ~ `day13`）。如果你写练习时卡住了，想看"官方参考实现长什么样"，可以：
...
- `src/tools/` —— 工具注册表与执行管线（Day4 引入）
- `src/core/` —— Agent loop、生命周期与取消（Day5、Day6 引入）、事件日志与消息派生（Day8 引入）、持久化与崩溃恢复（Day9 引入）、投影/查询/标题/Spill（Day10 引入）、系统提示词（Day11 引入）、Token 计量与预算（Day12 引入）、上下文压缩（Day13 引入）
